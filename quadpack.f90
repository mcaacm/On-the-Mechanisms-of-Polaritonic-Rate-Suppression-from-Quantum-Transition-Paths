...
!
  ...
  integer ( kind = 4 ), parameter :: limit = 50000
...
!  Compute final result.
!
  if ( last .gt. limit ) then
    ier = 1
    last = limit
  end if

  !WRITE(*,*) " last ", last, " limit ", limit, " ier ", ier
  result = sum ( rlist(1:last) )
...
!
  ...
  IF (ISNAN(result)) THEN
    WRITE(*,*) "QAWF checkpoint 0 is NaN"
  ELSE
    !WRITE(*,*) "QAWF is not NaN ", result
  END IF
  return
end
